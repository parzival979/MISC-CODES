...
//OPERATOR OVERLOAD TO MERGE THE LISTS
linkedListNode* operator + (linkedListNode &head1,linkedListNode &head2){
    linkedListNode *head11=&head1;
    linkedListNode *head21=&head2;

    int data1;
    while(head21 != NULL){
        data1=head21->getData();
        linkedListNode *currentPtr;
        currentPtr=head21;
        head21=head21->getPtr();
        delete currentPtr;
        head11=head11->addListNode(data1,head11);
    }
    return head11;
}
