...
h_trail = plot(x_robot(1), y_robot(1), 'b-', 'LineWidth', 2);

h_target = plot(circle_center(1) + circle_radius, circle_center(2), 'ro', 'MarkerFaceColor', 'r'); % Target point
...
for i = 1:N-1
    ...
    R = [cos(theta_robot(i)), -sin(theta_robot(i)); sin(theta_robot(i)), cos(theta_robot(i))];  
    ...
    robot_transformed(1,:) = robot_transformed(1,:) + x_robot(i);  
    robot_transformed(2,:) = robot_transformed(2,:) + y_robot(i);  
    
    set(h_robot, 'XData', robot_transformed(1,:), 'YData', robot_transformed(2,:));
    set(h_trail, 'XData', x_robot(1:i), 'YData', y_robot(1:i));
    
    % Update the position of the moving target on the circle
    set(h_target, 'XData', x_target, 'YData', y_target);  
...
legend('Circle', 'Robot Start', 'Robot Trajectory');
