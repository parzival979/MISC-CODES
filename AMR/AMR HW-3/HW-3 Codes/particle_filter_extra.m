...
for t = 2:num_steps
    ...
    distances_to_landmarks = sqrt((landmarks(:,1) - robot_true_pose(1)).^2 + (landmarks(:,2) - robot_true_pose(2)).^2);
    robot_visible_landmarks = find(distances_to_landmarks <= sensor_range);
    if isempty(robot_visible_landmarks)
    ...
    measurements = distances_to_landmarks(robot_visible_landmarks) + randn(length(robot_visible_landmarks), 1) * measurement_noise_std;
    ...
    for i = 1:num_particles
        ...
        particle_visible_landmarks = find(distances_particle <= sensor_range);
        
        common_landmarks = intersect(robot_visible_landmarks, particle_visible_landmarks);
        
        if isempty(common_landmarks)
            weights(i) = 1e-10;
        else
            expected_measurements = distances_particle(common_landmarks);
            robot_measurements = measurements(ismember(robot_visible_landmarks, common_landmarks));
            prob = normpdf(robot_measurements, expected_measurements, measurement_noise_std);
            weights(i) = prod(prob);
        end
    end
...
for t = 1:num_steps
    ...
    xlabel('X Position (m)');
    ylabel('Y Position (m)');
    title(['Particle Filter Localization with Sensor Range - Step ', num2str(t)]);
    ...
    viscircles(robot_true_poses(t,1:2), sensor_range, 'Color', 'g', 'LineStyle', '--');
